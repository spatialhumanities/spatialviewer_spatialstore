...
import de.i3mainz.ibr.export.FeatureDetailPage;
import de.i3mainz.ibr.export.Metadata;
import de.i3mainz.ibr.filter.Filter;
...
public class RestResource {
    ...
    @GET
    @Path("/{spatialcontext}/metadata")
    @Produces(MediaType.APPLICATION_OCTET_STREAM)
    public Response getMetadata(@Context HttpServletRequest req,
            @PathParam("spatialcontext") String spatialcontext) {
        return Metadata.getMetadataResponse(spatialcontext);
    }
...
*/
    ...
    @GET
    @Path("/{spatialcontext}/features/{feature}.xml")
    @Produces(MediaType.APPLICATION_XML)
    public Response exportFeature(@Context HttpServletRequest req,
            @PathParam("spatialcontext") String spatialcontext, @PathParam("feature") String feature) {
        Identification user = Identification.getUser(req);
        return Export.getFeatures(user, spatialcontext, "" + toInt(feature), null, null, "xml", null, toInt(null, null), toInt(null, null));
    }
    ...
    public Response postProjectfileOrDeleteIt(@FormParam("projectXML") String XML_Request,
        ...
        try {
            ...
            if (XML_Request != null) { // Neues Projekt anlegen
                // String von "unnÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¶tigen" Zeichen entfernen
				...
				// Gesamte Datenbank lÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¶schen und neu erzeugen
                // Datenbankverbindung erstellen und Datenbank lÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¶schen
                ...
                // Nur ein Projekt lÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¶schen
    ...
    /**
     * Returns json with queried values.
     *
     * @param req
     ...
     */
    @POST
    @Path("/{spatialcontext}/filter")
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.TEXT_PLAIN)
    public Response getFilter(@Context HttpServletRequest req,
            @PathParam("spatialcontext") String spatialcontext, @FormParam("json") String json) {
        return Filter.filter(spatialcontext, json);
    }
    //</editor-fold>

    //<editor-fold defaultstate="collapsed" desc="TRIPLESTORE">
    @POST
    @Path("/annotate")
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.TEXT_PLAIN)
    public Response annotate(@Context HttpServletRequest req,
            @FormParam("json") String json) {
        return TripleStoreConnect.annotate(json);
    }
    ...
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.TEXT_PLAIN)
    public Response query(@Context HttpServletRequest req,
    ...
    public Response exportHTML(@Context HttpServletRequest req, @PathParam("spatialcontext") String sc, @PathParam("feature") String fid) {
        return FeatureDetailPage.getPage(sc, fid);
    }

    @POST
    @Path("/export")
    @Produces(MediaType.APPLICATION_OCTET_STREAM)
    public Response export(@Context HttpServletRequest req, @FormParam("json") String json) {
        return de.i3mainz.ibr.export.Export.export(json);
    }
        //</editor-fold>
}
