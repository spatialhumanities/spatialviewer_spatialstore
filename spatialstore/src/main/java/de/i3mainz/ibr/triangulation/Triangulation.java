...
public class Triangulation {
	...
	public static boolean secondValidation(ArrayList<Point2D> polygon){
        // prüft jede linie eines polygons auf schnitte mit allen anderen 
        // linien des polygons, ausgenommen mit der 
        // nächsten und der vorhergehenden linie
		for (int i=0;i<polygon.size();i++){
    ...
    private static void printPointList(ArrayList<Point2D> list) {
        String out = "";
        for(Point2D point : list) {
            out += point.getX() + " " + point.getY() + ", ";
        }
        
        System.out.println("pointlist for WKT: " + out.substring(0, out.length()-2));
    }
